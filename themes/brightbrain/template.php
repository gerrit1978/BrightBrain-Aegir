...
function brightbrain_preprocess_maintenance_page(&$variables, $hook) {
  // When a variable is manipulated or added in preprocess_html or
  // preprocess_page, that same work is probably needed for the maintenance page
  // as well, so we can just re-use those functions to do that work here.
  brightbrain_preprocess_html($variables, $hook);
  brightbrain_preprocess_page($variables, $hook);
}
// */
...
function brightbrain_preprocess_page(&$variables, $hook) {
...
function brightbrain_preprocess_node(&$variables, $hook) {
  if ($variables['node']->type == 'page') {
    ...
    if (is_array($node->field_page_slogan) && count($node->field_page_slogan)) {
      $slogan_field = field_get_items('node', $node, 'field_page_slogan');
      $slogan = $slogan_field[0]['safe_value'];
    }
    ...
    if (isset($slogan)) {
      $markup .= '<div class="text"><div class="slogan"><h1>' . $node->title . '</h1><h2>' . $slogan . '</h2></div></div>';
    }
 ...
 */
/* -- Delete this line if you want to use this function
function brightbrain_preprocess_comment(&$variables, $hook) {
  $variables['sample_variable'] = t('Lorem ipsum.');
}
// */
 ...
 */
/* -- Delete this line if you want to use this function
function brightbrain_preprocess_region(&$variables, $hook) {
 ...
 */
/* -- Delete this line if you want to use this function
function brightbrain_preprocess_block(&$variables, $hook) {
 ...
 *
 */
function brightbrain_preprocess_field(&$variables) {
...
function brightbrain_menu_link($variables) {
  ...
  // define an array containing links for contact and about pages
  $general_links = array(
    'node/5' => $current_section . '/over-ons',
    'node/7' => $current_section . '/contact'
  );
  
  $current_class = isset($element['#original_link']['options']['attributes']['class'][0]) ? $element['#original_link']['options']['attributes']['class'][0] : '';
  
  // only show the HR links in HR section and MARKETING links in MARKETING section
  if ($current_class == 'hr' || $current_class == 'marketing') {
    if ($current_section != $current_class) {
      return;
    }
  }

  // check for "about" and "contact" links
  // currently: about = nid 5; contact = nid 7
  if ($current_class == 'general') {
    $href = $element['#href'];
    $alias = $general_links[$href];
    $output = l($element['#title'], $alias, $element['#localized_options']);
  }
